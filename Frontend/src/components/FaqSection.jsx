...
const FaqSection = () => {
  ...
  return (
    <section className="relative py-8 sm:py-10 md:py-12 lg:py-16 overflow-hidden bg-gradient-to-br from-[#0a0a1a] via-[#0f0a2a] to-[#0a1a2a]">
      ...
      <div className="container mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 2xl:px-24 relative z-10">
        ...
        >
          <motion.span 
            className="text-blue-400 font-semibold text-[10px] sm:text-xs tracking-widest uppercase bg-blue-500/20 backdrop-blur-sm px-3 sm:px-4 py-1 sm:py-1.5 rounded-full border border-blue-500/30 inline-flex items-center gap-1.5"
            whileHover={{ scale: 1.05 }}
            animate={{ y: [0, -3, 0] }}
            transition={{ duration: 2, repeat: Infinity }}
          >
          ...
          <motion.h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-white mt-2 leading-tight">
            Frequently Asked{' '}
            <motion.span 
              className="bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent inline-block"
              animate={{ backgroundPosition: ['0% 50%', '100% 50%', '0% 50%'] }}
        ...
        <motion.div 
          ...
          transition={{ duration: 0.5, delay: 0.1 }}
          viewport={{ once: true }}
          className="max-w-2xl mx-auto mb-5 sm:mb-6 md:mb-8"
        >
          <div className="relative">
            ...
            <input
              ...
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full bg-white/5 backdrop-blur-sm border border-white/10 rounded-lg py-2 sm:py-2.5 pl-9 sm:pl-10 pr-3 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-transparent transition-all duration-300 text-xs sm:text-sm"
            />
          </div>
        </motion.div>
        ...
        {/* FAQ List - Compact */}
        <motion.div 
          className="max-w-4xl mx-auto space-y-2 sm:space-y-2.5"
        >
          {filteredFaqs.length > 0 ? (
            filteredFaqs.map((faq, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className={`bg-white/5 backdrop-blur-sm rounded-lg sm:rounded-xl border transition-all duration-300 overflow-hidden ${
                  openIndex === index 
                    ? 'border-blue-500/50 shadow-lg shadow-blue-500/10' 
                    : 'border-white/10 hover:border-white/20'
                }`}
                  ...
                  {openIndex === index && (
                    ...
                    >
                      <div className="px-3 sm:px-4 pb-3 sm:pb-4">
                        <div className="h-px bg-gradient-to-r from-blue-500/20 via-purple-500/20 to-pink-500/20 mb-2 sm:mb-3"></div>
            ...
            <div className="text-center py-8">
              ...
              <button 
                onClick={() => setSearchTerm('')}
                className="text-blue-400 hover:text-blue-300 mt-1.5 text-xs"
              >
                Clear search
              </button>
            </div>
        ...
        <motion.div 
          className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 mt-6 sm:mt-8 md:mt-10"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 0.3 }}
          viewport={{ once: true }}
        >
          <p className="text-blue-200/60 text-xs sm:text-sm mr-1">Still have questions?</p>
          
          <motion.a
            href="/contact"
            whileHover={{ scale: 1.04, boxShadow: "0 15px 30px rgba(1,173,240,0.3)", y: -2 }}
            whileTap={{ scale: 0.95 }}
            className="inline-flex items-center gap-1.5 px-4 sm:px-5 py-1.5 sm:py-2 rounded-full bg-[#01adf0] text-white font-semibold hover:shadow-xl transition-all duration-300 text-[10px] sm:text-xs"
          >
